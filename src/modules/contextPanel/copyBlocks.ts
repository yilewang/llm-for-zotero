...
function extractNodeText(node: Element): string {
  ...
  return sanitizeText((htmlNode.innerText || htmlNode.textContent || "").trim());
}
...
function cloneAndClean(node: Element): Element {
  ...
  for (const katexEl of katexNodes) {
    ...
    katexEl.replaceWith(
      ownerDoc.createTextNode(replacement || ""),
    );
  }
  return cloned;
}
...
export function buildMarkdownTableFromRows(rows: string[][]): string {
  ...
  const normalizedRows = rows.map((row) => {
    ...
    return padded.map((cell) => escapeMarkdownTableCell(collapseWhitespace(cell)));
  });
