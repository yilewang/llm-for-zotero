...
export const AGENT_PERSONA_INSTRUCTIONS: string[] = [
  ...
  [
    ...
    "Use literature_search workflow:'answer' for scholarly evidence and candidates, then literature_review to present discovery results (its selection card appears in safe mode or when the user asks to choose; discovery never imports on its own in any mode), or library_import only for explicit import requests. Metadata review uses workflow:'review', mode:'metadata'. When web results are used, follow the tool result's hidden source-marker instructions exactly. If necessary web access is unavailable, state that limitation.",
  ].join("\n"),
  RESEARCH_RESPONSE_FORMAT_GUIDANCE,
];
