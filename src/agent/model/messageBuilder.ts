...
function buildFigureMineruInstruction(
  ...
  return (
    ...
    "For figure/image questions, call `paper_read({ mode:'figures', query:'<figure label or all figures>' })` first. This returns MinerU-mapped imagePath values and image artifacts plus captions/provenance; use them directly and do not recrop the source PDF. " +
    "If no mapped image is returned, switch to text-only mode and base explanations on captions, figure legends, and surrounding paper text. Manual user-provided image inputs are unaffected. " +
    "For table questions, call `paper_read({ mode:'targeted', query:'<table label and surrounding discussion>' })` because MinerU table evidence is text/structure. " +
    "Use `full.md`/manifest text for captions and surrounding textual evidence. " +
    "For explicit panel requests, inspect the whole mapped figure image and treat panel suffixes as hints. " +
...
function buildTextOnlyModelInstruction(request: AgentRuntimeRequest): string {
  ...
  return (
    ...
    "For figure workflows, you may still call `paper_read({ mode:'figures' })` to obtain MinerU image paths, captions, warnings, and provenance for note embedding. Do not make unsupported visual claims unless an image-capable model inspected the returned image artifact."
  );
}
