...
const zhCN: Record<string, string> = {
  ...
  "Uses vector embeddings for meaning-aware search. When disabled, only keyword matching (BM25) is used.": "使用向量嵌入进行语义理解搜索。关闭后，仅使用关键词匹配（BM25）。",
  ...
  "File I/O not available": "文件读写功能不可用",
  "Directory not found: %path%": "未找到目录：%path%",
  "Notes Directory": "笔记目录",
  "Configure a local directory for saving notes as files. Note format and templates are managed through skills — type": "配置一个本地目录，用于将笔记保存为文件。笔记格式和模板通过技能管理，输入",
  "in chat to see available skills, or edit skill files directly.": "可在聊天中查看可用技能，或直接编辑技能文件。",
  "Nickname": "昵称",
  "e.g., Obsidian, Logseq, My Notes": "例如：Obsidian、Logseq、我的笔记",
  "How you refer to this directory — the agent will recognize it when you mention it": "你对这个目录的称呼，agent 会在你提到它时识别出来",
  "Notes Directory Path": "笔记目录路径",
  "/path/to/notes or C:\\path\\to\\notes": "例如 /path/to/notes 或 C:\\path\\to\\notes",
  "Absolute path to the directory where notes are saved as files": "保存笔记文件的目录绝对路径",
  "Folder for images, relative to vault root (e.g., assets, Notes/imgs)": "用于存放图片的文件夹，相对于根目录（例如 assets、Notes/imgs）",
  "Exclude PDFs by Filename": "按文件名排除 PDF",
  "Comma-separated patterns. PDFs with matching filenames will be skipped. Wrap in /slashes/ for regex.": "用逗号分隔多个模式。文件名匹配的 PDF 将被跳过。正则请用 /斜杠/ 包裹。",
  ...
  "Add provider": "添加服务商",
  "+ Add Provider": "+ 添加服务商",
  "Maximum %n providers": "最多支持 %n 个服务商",
  ...
  "Preset uses OpenAI's official Responses endpoint.": "该预设使用 OpenAI 官方 Responses 端点。",
  "Preset uses Gemini's native generateContent endpoint.": "该预设使用 Gemini 原生 generateContent 端点。",
  "Preset uses Anthropic's native Messages API.": "该预设使用 Anthropic 原生 Messages API。",
  "Preset uses MiniMax's recommended Anthropic-compatible endpoint.": "该预设使用 MiniMax 推荐的 Anthropic 兼容端点。",
  "Preset uses GLM's Claude-compatible endpoint for agent tool use.": "该预设使用 GLM 面向 Agent 工具调用的 Claude 兼容端点。",
  "Preset uses DeepSeek's official API base (v1).": "该预设使用 DeepSeek 官方 API 基础地址（v1）。",
  "Preset uses xAI's official Responses endpoint.": "该预设使用 xAI 官方 Responses 端点。",
  "Preset uses DashScope's compatible-mode API base (v1).": "该预设使用 DashScope 的 compatible-mode API 基础地址（v1）。",
  "Preset uses Moonshot's international API. Use api.moonshot.cn for China.": "该预设使用 Moonshot 国际版 API。在中国请使用 api.moonshot.cn。",
  "Uses GitHub Copilot via device login. Requires an active Copilot subscription.": "通过设备登录使用 GitHub Copilot。需要有效的 Copilot 订阅。",
  "Codex Responses": "Codex Responses",
  "Responses API": "Responses API",
  "OpenAI-Compatible Chat": "OpenAI 兼容 Chat",
  "Anthropic Messages": "Anthropic Messages",
  "Gemini Native": "Gemini 原生",
  "Web Sync (ChatGPT / DeepSeek)": "Web 同步（ChatGPT / DeepSeek）",
  "Use ChatGPT/Codex auth with the Codex Responses endpoint.": "使用 ChatGPT/Codex 认证并调用 Codex Responses 端点。",
  "Use OpenAI-style Responses APIs with tool calls and direct file input.": "使用 OpenAI 风格的 Responses API，支持工具调用和直接文件输入。",
  "Use OpenAI-compatible chat/completions APIs with tool calls, but without direct file input.": "使用 OpenAI 兼容的 chat/completions API，支持工具调用，但不支持直接文件输入。",
  "Use Anthropic's native Messages API with streaming tool use and image input.": "使用 Anthropic 原生 Messages API，支持流式工具调用和图片输入。",
  "Use Gemini's native generateContent API with streaming tool calls and image input.": "使用 Gemini 原生 generateContent API，支持流式工具调用和图片输入。",
  "Relay questions to ChatGPT, DeepSeek via the browser extension web-sync bridge.": "通过浏览器扩展的 web-sync 桥接，将问题转发到 ChatGPT、DeepSeek。",
  ...
  "Using API key from your %provider% provider": "使用你配置的 %provider% 服务商 API 密钥",
  "API key configured": "已配置 API 密钥",
  "No %provider% provider found. Enter an API key for embeddings.": "未找到 %provider% 服务商配置。请输入用于嵌入的 API 密钥。",
  "Estimated cost": "预估成本",
