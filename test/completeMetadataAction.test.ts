...
function createStubTool<TInput extends Record<string, unknown>, TResult>(
  ...
  return {
    ...
    execute: async (input, context) => {
      if (spec.executionClass === "external_effect")
        assert.equal(
          context.executionAuthority,
          "user",
          "Slash action writes require an actual reviewed proposal in every mode",
        );
...
for (const mode of ["safe", "auto", "yolo"] as const)
  describe(`completeMetadata action (${mode})`, function () {
    ...
    it("supports legacy itemId input and only fills empty fields", async function () {
      ...
      const metadata =
        (operations[0]?.metadata as Record<string, unknown>) || {};
